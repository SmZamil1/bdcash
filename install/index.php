...
if ($action == 'requirements') {
	...
	foreach ($dirs as $dir) {
        // Auto-fix permission if needed
        if (substr(sprintf('%o', fileperms($dir)), -4) < '0775') {
            @chmod($dir, 0775);
            clearstatcache();
        }
...
MIX_PUSHER_APP_CLUSTER='${PUSHER_APP_CLUSTER}'";
	...
	if (@$response['error'] == 'ok') {
		try {
            ...
            $installedLogFile = dirname(__DIR__, 1) . '/installed';
            file_put_contents($installedLogFile, 'Installation Completed on '.date('Y-m-d H:i:s'));
		} catch (Exception $e) {
...
<body>
	<header class="py-3 border-bottom border-primary bg--dark">
		<div class="container">
			<div class="d-flex align-items-center justify-content-between header gap-3">
				<img class="logo" src="../assets/images/logo_icon/favicon.png" alt="Logo">
						...
						<div class="box-item">
							...
							if ($action == 'result') {
								...
								if (@$response['error'] == 'ok') {
									...
									echo '<p class="text-primary lead my-5 review-alert">Installation process completed successfully.</p>';
							...
							?>
								<div class="item">
									<h4 class="subtitle">System Installation Terms</h4>
									<p>Please ensure your server meets all requirements before proceeding. Unauthorized distribution or modification of this system without proper license is prohibited.</p>
								</div>
								<div class="item">
									<h5 class="subtitle font-weight-bold">Permissions:</h5>
									<ul class="check-list">
										<li> Install on your supported server. </li>
										<li> Configure inputs as required. </li>
									</ul>
								</div>
	...
	<footer class="py-3 text-center bg--dark border-top border-primary">
		<div class="container">
			<p class="m-0 font-weight-bold">&copy;<?php echo Date('Y') ?> - All Right Reserved by <a href="#">HyipX</a></p>
		</div>
	</footer>
	<script src="../assets/global/js/bootstrap.bundle.min.js"></script>
	 <script src="../assets/global/js/firebase/firebase-8.3.1.js"></script>
</body>
</html>